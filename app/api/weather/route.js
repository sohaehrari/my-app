export async function GET(request) {
  ...
  if (!apiKey) {
    ...
    return Response.json(
      {
        error:
          "Weather API key is not configured. Check your .env.local file.",
      },
      { status: 500 }
    );
  }
  ...
  if (!city && (!lat || !lon)) {
    return Response.json(
      {
        error:
          "Please provide a city or latitude and longitude.",
      },
      { status: 400 }
    );
  }
  ...
  if (!response.ok) {
    console.error("OpenWeather current error:", data);
    ...
    if (response.status === 401) {
      return Response.json(
        {
          error:
            "Invalid OpenWeather API key. Check WEATHER_API_KEY in .env.local.",
        },
        { status: 401 }
      );
    }
  ...
  return Response.json(
    {
      error:
        error?.message ||
        "Something went wrong while getting weather.",
    },
    { status: 500 }
  );
